...
func 말이동정보만들기(윷짝a :윷짝, m :말)->말들이동정보:
	...
	if m.난말인가() :
		return 말들이동정보.new()
	if m.달말인가() and 윷짝a.result_value > 0:
...
func 새로말달정보만들기(윷짝a :윷짝, m :말)->말들이동정보:
	var 결과 = 말들이동정보.new()
	결과.이동과정눈번호들 = 길.말이동과정찾기(-1,윷짝a.result_value)
...
func 판위의말이동할정보만들기(윷짝a :윷짝, m :말)->말들이동정보:
	var 결과 = 말들이동정보.new()
	결과.이동할말들 = 업은말들얻기(m)
	if 윷짝a.result_value < 0: # 뒷도개걸 처리
		if m.지나온눈번호들.size() <= -윷짝a.result_value: #판에서 빼서 놓을 말로 돌아간다.
			for i in m.지나온눈번호들:
			...
			결과.이동성공 = true
			return 결과
		for i in range(윷짝a.result_value,0): # 업은말의 첫말의 지나온눈번호들에서 빼면서 뒤로 이동한다.
	...
	결과.이동과정눈번호들 = 길.말이동과정찾기(m.마지막눈번호(),윷짝a.result_value)
