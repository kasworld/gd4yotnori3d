...
func 말쓰기(윷짝a :윷짝, m :말)->말들이동정보:
	...
	if 업힌말인가(m):
		return 말들이동정보.new()
	return 판위의말이동하기(윷짝a, m)

func 새로말달기(윷짝a :윷짝, m :말)->말들이동정보:
	var 결과 = 말들이동정보.new()
	결과.말이동과정눈번호 = 길.말이동과정찾기(-1,윷짝a.결과얻기())
...
func 판위의말이동하기(윷짝a :윷짝, m :말)->말들이동정보:
	var 결과 = 말들이동정보.new()
	...
	결과.말이동과정눈번호 = 길.말이동과정찾기(m.마지막눈번호(),윷짝a.결과얻기())
