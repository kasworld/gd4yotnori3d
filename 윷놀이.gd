...
func 말이동하기() -> void:
	...
	말들이동정보g.다음편으로넘어가나 = (not 윷짝1.한번더던지나()) and 말들이동정보g.잡힐말들.size() == 0
	if not 말들이동정보g.이동성공:
