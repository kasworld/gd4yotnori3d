...
func 말이동하기() -> void:
	...
	if m != null and m.놓을말인가():
		$"말판/달말통".말빼기(m)
	말들이동정보g = 윷던진편.말쓰기(윷짝1, m)
	말들이동정보g.다음편으로넘어가나 = (not 윷짝1.한번더던지나()) and 말들이동정보g.잡힌말들.size() == 0
	if not 말들이동정보g.성공:
		진행사항.text = "%d %s %s 이동할 말이 없습니다.\n" % [윷짝1.던진횟수얻기(), 윷던진편 , 윷짝1 ] + 진행사항.text
		이동애니메이션후처리하기()
		return
	var 좌표들 = 눈번호들을좌표로(말들이동정보g.말이동과정눈번호)
	if 말들이동정보g.새로단말 != null:
		좌표들.push_front(윷던진편.길.놓을길시작 )
	if 말들이동정보g.놓을말로돌아간말들.size() != 0:
		진행사항.text = "    %s 놓을말로되돌아갔습니다.\n" % [말들이동정보g.놓을말로돌아간말들 ] + 진행사항.text
		좌표들.push_back(윷던진편.길.놓을길시작 )
		#$"말판/달말통".말들넣기(말들이동정보g.놓을말로돌아간말들)
	if 말들이동정보g.난말들.size() != 0:
		진행사항.text = "    %s 났습니다.\n" % [말들이동정보g.난말들 ] + 진행사항.text
		좌표들.push_back(윷던진편.길.나는길끝 )
		#$"말판/난말통".말들넣기(말들이동정보g.난말들)
	if 말들이동정보g.잡힌말들.size() != 0 :
		진행사항.text = "    %s 을 잡아 한번더 던진다. \n" % [ 말들이동정보g.잡힌말들 ] + 진행사항.text
		#$"말판/달말통".말들넣기(말들이동정보g.잡힌말들)
